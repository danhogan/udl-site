<template>
    <div>
        <!-- I really wanted to get this out before opening day, please don't kill me for this disgrace -->
...
const daRosters = [
    ...
    },
    {
      ...
      "roster": [
        ...
        _.times(daRosters.length, (i) => {
            let thisRoster = daRosters[i].roster
            let rosterLength = thisRoster.length

            let thisFormattedRoster = []
            _.times(rosterLength, (x) => {
                let playerAge = 0
                let birthday = thisRoster[x].birthday
                ...
                let flooredAge = Math.floor(diffDays/365)

                if(flooredAge < minAge){minAge = flooredAge}
                if(flooredAge > maxAge){maxAge = flooredAge}

                thisFormattedRoster.push({
                    // playerId: thisRoster[x].playerId,
                    name: thisRoster[x].name,
                    age: flooredAge
                })
            })

            allTeams.push({
                team: daRosters[i].team,
                roster: thisFormattedRoster
            })
        })
...
_.times(20, (w) => {
        let oneTeam = allTeams[w]
        let distribution = []
        _.times(oneTeam.roster.length, (q) => {
            let thisAge = distribution[oneTeam.roster[q].age]
            if(thisAge > 0){
                distribution[oneTeam.roster[q].age]++
            } else {
                distribution[oneTeam.roster[q].age] = 1
            }
        })
        // console.log('distribution:', distribution)

        let formattedDistribution = []
        _.each(_.range(minAge, maxAge+1), (e) => {
            let value = distribution[e] > 0 ? distribution[e] : 0
            formattedDistribution.push({
                key: e,
                value: value
            })
        })
        // console.log('formattedDistribution:', formattedDistribution)
        ...
        var svg = d3.select("#distChart").append("svg")
